...
def show_result(num_epoch, show = False, save = False, path = 'result.png'):
    ...
    output = sess.run(G_z,{z: z_})
    grid_size = 5
    fig, ax = plt.subplots(grid_size, grid_size, figsize=(grid_size, grid_size))
    for x, y in ((i, j) for i in range(grid_size) for j in range(grid_size)):
    ...
    for i in range(25):
        ...
        y = i % 5
        ax[x, y].cla()
        ax[x, y].imshow(np.reshape(output[i], (28, 28)), cmap='gray')

    label = 'Epoch {0}'.format(num_epoch)
    fig.text(0.5, 0.04, label, ha='center')
    plt.savefig(path)

    # Show generated batch images at 10, 20, 50 and 100 epochs
